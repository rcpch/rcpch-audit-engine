...
def test_audit_centre_clinician(client, e12_user_factory, seed_groups_fixture):
    ...
    response_same_org = client.get(
        f"/organisation/{TEST_USER_ORGANISATION.id}/epilepsy12_user_list/"
    )
    response_diff_org = client.get(
        f"/organisation/{DIFFERENT_USER_ORGANISATION.id}/epilepsy12_user_list/"
    )

    assert (
        response_same_org.status_code == 200
    ), f"User from {TEST_USER_ORGANISATION} requested to see Users list from their own organisation, but not receiving 200 response"

    assert (
        response_diff_org.status_code == 403
    ), f"User from {TEST_USER_ORGANISATION} requested to see Users list from different organisation ({DIFFERENT_USER_ORGANISATION}). Expecting 403, receiving {response_diff_org.status_code}"


@pytest.mark.xfail(
    reason="The view does raise PermissionDenied and returns a 403 template, however the response code is still 200"
)
@pytest.mark.django_db
def test_audit_centre_clinician_cases_list_access(
    ...
    with pytest.raises(PermissionDenied) as excinfo_same:
        res_same_org = client.get(
            reverse("cases", kwargs={"organisation_id": ORGANISATION_TEST_USER.id})
        )

    with pytest.raises(PermissionDenied) as excinfo_other:
        res_other_org = client.get(
            reverse("cases", kwargs={"organisation_id": ORGANISATION_OTHER.id})
        )

    assert (
        str(excinfo_same.value) == f"PermissionError: [Errno 13] {res_same_org}"
    )  # "User from {ORGANISATION_TEST_USER} requesting cases list from {ORGANISATION_TEST_USER}. Expecting 200 status_code, receiving {res_same_org.status_code}"
    assert (
        str(excinfo_other.value) == f"PermissionError: [Errno 13] {res_other_org}"
    )  # User from {ORGANISATION_TEST_USER} requesting cases list from {ORGANISATION_OTHER}. Expecting 403 status_code, receiving {res_other_org.status_code}"

    # res_other_org.status_code == 403, f"User from {ORGANISATION_TEST_USER} requesting cases list from {ORGANISATION_OTHER}. Expecting 403 status_code, receiving {res_other_org.status_code}"
