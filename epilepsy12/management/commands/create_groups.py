...
class CreateGroupsSeed:
    ...
    def initialize_permissions(self, apps, schema_editor):
        """
    ...
    def add_permissions_to_group(self, permissions_list, group_to_add):
    ...
    def add_permissions_to_existing_groups(self):
        for group in GROUPS:
            ...
            if group == EPILEPSY12_AUDIT_TEAM_VIEW_ONLY:
                ...
                self.add_permissions_to_group(
                    CreateGroupsSeed.EPILEPSY12_AUDIT_TEAM_VIEW_ONLY_PERMISSIONS,
                    newGroup,
                )
                # basic permissions
                self.add_permissions_to_group(CreateGroupsSeed.VIEW_PERMISSIONS, newGroup)

            elif group == EPILEPSY12_AUDIT_TEAM_EDIT_ACCESS:
                # custom permissions
                self.add_permissions_to_group(
                    CreateGroupsSeed.EPILEPSY12_AUDIT_TEAM_EDIT_ACCESS_PERMISSIONS,
                    newGroup,
                )
                # basic permissions
                self.add_permissions_to_group(CreateGroupsSeed.VIEW_PERMISSIONS, newGroup)
                self.add_permissions_to_group(CreateGroupsSeed.EDITOR_PERMISSIONS, newGroup)

            elif group == EPILEPSY12_AUDIT_TEAM_FULL_ACCESS:
                # custom permissions
                self.add_permissions_to_group(
                    CreateGroupsSeed.EPILEPSY12_AUDIT_TEAM_FULL_ACCESS_PERMISSIONS,
                    newGroup,
                )
                # basic permissions
                self.add_permissions_to_group(CreateGroupsSeed.VIEW_PERMISSIONS, newGroup)
                self.add_permissions_to_group(CreateGroupsSeed.EDITOR_PERMISSIONS, newGroup)
                self.add_permissions_to_group(
                    CreateGroupsSeed.FULL_ACCESS_PERMISSIONS, newGroup
                )

            elif group == TRUST_AUDIT_TEAM_VIEW_ONLY:
                # custom permissions
                self.add_permissions_to_group(
                    CreateGroupsSeed.TRUST_AUDIT_TEAM_VIEW_ONLY_PERMISSIONS, newGroup
                )
                # basic permissions
                self.add_permissions_to_group(CreateGroupsSeed.VIEW_PERMISSIONS, newGroup)
                ...
                self.add_permissions_to_group(
                    CreateGroupsSeed.TRUST_AUDIT_TEAM_EDIT_ACCESS_PERMISSIONS, newGroup
                )
                # basic permissions
                self.add_permissions_to_group(CreateGroupsSeed.VIEW_PERMISSIONS, newGroup)
                self.add_permissions_to_group(CreateGroupsSeed.EDITOR_PERMISSIONS, newGroup)
                ...
                # custom permissions
                self.add_permissions_to_group(CreateGroupsSeed.TRUST_AUDIT_TEAM_VIEW_ONLY_PERMISSIONS, newGroup
                )
                self.add_permissions_to_group(CreateGroupsSeed.TRUST_AUDIT_TEAM_EDIT_ACCESS_PERMISSIONS, newGroup
                )
                self.add_permissions_to_group(CreateGroupsSeed.TRUST_AUDIT_TEAM_FULL_ACCESS_PERMISSIONS, newGroup
                )
                # basic permissions
                self.add_permissions_to_group(CreateGroupsSeed.VIEW_PERMISSIONS, newGroup)
                self.add_permissions_to_group(CreateGroupsSeed.EDITOR_PERMISSIONS, newGroup)
                self.add_permissions_to_group(CreateGroupsSeed.FULL_ACCESS_PERMISSIONS, newGroup)

            elif group == PATIENT_ACCESS:
                # custom permissions
                self.add_permissions_to_group(CreateGroupsSeed.PATIENT_ACCESS_PERMISSIONS, newGroup)
                # basic permissions
                self.add_permissions_to_group(CreateGroupsSeed.VIEW_PERMISSIONS, newGroup)
    ...
    def create_groups(self):
        ...
        for group in GROUPS:
            if not Group.objects.filter(name=group).exists():
                ...
                if group == EPILEPSY12_AUDIT_TEAM_VIEW_ONLY:
                    # custom permissions
                    self.add_permissions_to_group(CreateGroupsSeed.
    ...
    def delete_and_reallocate_permissions(self):
        for group in GROUPS:
