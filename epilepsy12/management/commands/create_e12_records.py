...
from ...models import FirstPaediatricAssessment, EpilepsyContext, MultiaxialDiagnosis, Syndrome, Episode, Keyword, Comorbidity, Assessment, Site, Organisation, Management, AntiEpilepsyMedicine, Case, AuditProgress, Registration, KPI, Investigations, SyndromeEntity, EpilepsyCauseEntity, ComorbidityEntity, MedicineEntity
...
def create_epilepsy_context(registration_instance):
    ...
    if not hasattr(registration_instance, 'epilepsycontext'):
        return EpilepsyContext.objects.create(
            ...
            previous_acute_symptomatic_seizure=OPT_OUT_UNCERTAIN[randint(
                0, 2)][0],
...
def create_assessment(registration_instance):
    ...
    if not hasattr(registration_instance, 'assessment'):
        assessment = Assessment.objects.create(
            ...
            childrens_epilepsy_surgical_service_referral_made=bool(
                getrandbits(1)),
        ...
        print(
            f'Assessment exists for {registration_instance.case}. Skipping...')
...
def create_investigations(registration_instance):
    ...
    if not hasattr(registration_instance, 'investigations'):
        ...
        print(
            f'Investigations exist for {registration_instance.case}. Skipping...')
    ...
    if not hasattr(registration_instance, 'management'):
        ...
        if management.has_an_aed_been_given:
            for count_item in range(1, randint(1, 3)):
                ...
                random_medicine = MedicineEntity.objects.filter(
                    is_rescue=False).order_by("?").first()
                ...
                if registration_instance.case.sex == 2 and hasattr(antiepilepsy_medicine, 'medicine_entity'):
                    if hasattr(antiepilepsy_medicine, 'medicine_entity') and antiepilepsy_medicine.medicine_entity is not None:
                        if antiepilepsy_medicine.medicine_entity.medicine_name == "Sodium valproate":
        ...
        print(
            f'Management exists for {registration_instance.case}. Skipping...')
