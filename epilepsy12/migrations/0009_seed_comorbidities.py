...
from ..constants.comorbidities import *
...
path = settings.BASE_DIR / "epilepsy12" / "constants" / "comorbidities.json"
...
def seed_comorbidities(apps, schema_editor):
    ...
    if ComorbidityList.objects.count() >= 312:
        logger.info(
            f"{ComorbidityList.objects.count()} Comorbidities already exist. Skipping..."
        )
        return
    # ecl = '<< 35919005'
    # comorbidity_choices = fetch_ecl(ecl)

    for index, comorbidity_choice in enumerate(comorbidity_data):
    ...
    if not ComorbidityList.objects.filter(conceptId="-1").exists():
        ComorbidityList.objects.create(
            conceptId="-1", term="Other", preferredTerm="Other"
        )
