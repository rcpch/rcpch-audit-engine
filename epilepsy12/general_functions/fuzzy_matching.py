...
def scan_for_keywords(phrase: str, match_list) -> list:
    ...
    filtered_words = list(filter(None, re.findall(keywords_string, phrase.lower())))

    return_vals = []
    for word in filtered_words:
        return_vals.append(next(item for item in match_list if item.keyword == word))
    return return_vals


def fuzzy_scan_for_keywords(phrase: str, match_list) -> list:
    """
    Returns a list of matched keywords against a phrase. Uses the rapidfuzz dependency to match against keywords supplied
    as match list
    This function iterates through the keyword match list, returning a list of fuzzy matches
    """
    keywords = []
    for index, keyword in enumerate(match_list):
